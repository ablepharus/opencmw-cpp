...
public:
    ...
    void runWorker() {
        std::lock_guard<std::mutex> m{_setup};
        std::mutex foo;
        ...
        std::unique_lock<std::mutex>  lock{foo};
    ...
    void startBroker() {
        std::lock_guard<std::mutex> l{_setup};
        std::lock_guard<std::mutex> ltt{_mutex};
        std::condition_variable cv;
        std::mutex _m;
        std::unique_lock<std::mutex> lock{_m};
        bool ready{false};
